...
namespace ecs {
    ...
    void ComponentManager::JoueurMouvement(const SDL_Scancode keys[], Entity &entity){
        ...
        if (keystates[keys[0]]) vel.y = -1; // HAUT
        if (keystates[keys[1]]) vel.y = 1; // BAS
        if (keystates[keys[0]] && keystates[keys[1]]) vel.y = 0; //HAUT ET BAS

        if (keystates[keys[2]]) vel.x = -1; // GAUCHE
        if (keystates[keys[3]]) vel.x = 1; // DROITE
        if (keystates[keys[2]] && keystates[keys[3]]) vel.x = 0; // GAUCHE ET DROITE
